<?php
	if (isset($_POST["submit"])) {
		...
		if ($name == "" | $username =="" | $password =="" | $email == "" | $phone == "" | $department == "" | $idno == "" | $address == "") {
        ...
        if(mysqli_num_rows($sql2_u) > 0){
            ...
            $vkey = md5(time() . $username);
            $insert = mysqli_query($link, "INSERT INTO t_registration (name, username, password, email, phone, department, idno, address, utype, photo) 
											VALUES ('$name', '$username', '$password', '$email', '$phone', '$department', '$idno', '$address', '$utype', '$photo')");
            if($insert){
